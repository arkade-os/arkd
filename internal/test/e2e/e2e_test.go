...
// TestEagerForfeitSurvivesWalletRotation verifies that signing forfeit txs at
// collection time (this PR) closes the unpunishable-fraud window opened by a
// signer key rotation.
//
// On the pre-PR behavior the operator half of a forfeit was added lazily, at
// fraud-broadcast time; after a hard rotation (no deprecated key) the wallet
// signed the old-key forfeit leaf with the new key, the tx could not be
// finalized, and the fraud went unpunished. Here the forfeit is already
// operator-signed when it is collected (with the then-current/old key), so the
// stored tx stays broadcast-ready across the rotation: when the user
// fraudulently unrolls the forfeited vtxo the server still broadcasts the
// forfeit and claims it.
//
// The scenario is identical to the pre-PR repro (hard rotation, no deprecated
// key); only the expected outcome is inverted.
