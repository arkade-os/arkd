...
//
// ExpiresAt is deliberately not restored, since unrollVtxo zeroes it and the
// original value is gone, so a retracted vtxo keeps a zero expiry here where the
// SQL backends keep the value they never cleared.
