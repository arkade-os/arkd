...
-- generated by sqlc for @with_extension, @with_after, @after_ts,
-- @with_before and @before_ts bind to the correct positions after the
-- sqlite driver expands the slice. The generated `?N` markers refer to
-- absolute parameter positions, so anything bound after the slice shifts
-- once len(txids) > 1.
--
-- That is why this query has no @lim even though SelectOffchainTxs does.
-- A LIMIT parameter here would sit after the slice and break exactly that
-- way, so the caller batches this query and bounds the accumulated result
-- in Go instead. See trimToScanLimit in sqlite/offchain_tx_repo.go.
