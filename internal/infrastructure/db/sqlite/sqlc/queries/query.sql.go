...
func (q *Queries) SelectVtxosByArkTxidRecursive(ctx context.Context, initialTxid string) ([]SelectVtxosByArkTxidRecursiveRow, error) {
	rows, err := q.db.QueryContext(ctx, selectVtxosByArkTxidRecursive, initialTxid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
