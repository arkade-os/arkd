...
type mockTxDecoder struct {
	decodeFn func(tx string) (string, []ports.TxIn, []ports.TxOut, error)
}

func (m *mockTxDecoder) DecodeTx(tx string) (string, []ports.TxIn, []ports.TxOut, error) {
	return m.decodeFn(tx)
}
...
// TestFinalizePendingTxProjection tests the projection handler that runs when
// an offchain tx is finalized (Requested -> Accepted -> Finalized).
//
// When finalization happens after the tx's expiry timestamp has passed, the
// handler needs to check whether the input vtxos have been swept on-chain by
// the server. If any input is swept, the newly created output vtxos must also
// be marked as swept (they can't be spent on-chain since their inputs are gone).
//
// Two cases:
//   - Input vtxos were swept by the server -> output vtxos must be marked swept.
...
func TestFinalizePendingTxProjection(t *testing.T) {
	...
	// The mock decoder is swapped per-subtest to return unique outpoints.
	...
	tests := []struct {
		...
		{
			// The server already swept the inputs on-chain (round expired and
			// the server reclaimed the funds). Finalizing the offchain tx at
			// this point should propagate the swept status to the outputs.
			name:        "finalize_pending_tx_after_swept_by_server",
			inputSwept:  true,
			expectSwept: true,
		},
		{
			// The round expired but the server has NOT swept the inputs yet.
			// The outputs are still valid on-chain so they should not be
			// marked as swept.
			name:        "finalize_pending_tx_after_expired_not_swept",
			inputSwept:  false,
			expectSwept: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Fresh db per subtest for clean isolation.
			dbDir := t.TempDir()
			...
			// Generate unique input/output outpoints per subtest.
			in1 := domain.Outpoint{Txid: randomString(32), VOut: 0}
			in2 := domain.Outpoint{Txid: randomString(32), VOut: 0}
			outTxid := randomString(32)

			// Configure the mock decoder to map the ark tx to our test
			// inputs and outputs. The projection handler calls DecodeTx on
			// the finalized ark tx to discover which vtxos are consumed
			// (inputs) and which are created (outputs).
			decoder.decodeFn = func(tx string) (string, []ports.TxIn, []ports.TxOut, error) {
				return outTxid, []ports.TxIn{in1, in2}, []ports.TxOut{
					{Amount: 1000, PkScript: outputPkScript},
				}, nil
			}

			// Pre-populate the input vtxos in the store. The Swept flag
			// simulates whether the server has already swept them on-chain.
			inputVtxos := []domain.Vtxo{
				{
					Outpoint:           in1,
					PubKey:             pubkey,
					Amount:             500,
					Swept:              tt.inputSwept,
					RootCommitmentTxid: rootCommitmentTxid,
					CommitmentTxids:    []string{rootCommitmentTxid},
				},
				{
					Outpoint:           in2,
					PubKey:             pubkey,
			...
			// The expiry timestamp is in the past so the projection handler
			// will enter the "expired at finalization" code path and check
			// whether the input vtxos are swept.
			expiredTimestamp := time.Now().Add(-1 * time.Hour).Unix()
			txId := randomString(32)

			// Walk the offchain tx through its full lifecycle:
			// Requested -> Accepted (with expired timestamp) -> Finalized.
			// Saving these events triggers the projection handler async.
			events := []domain.Event{
				domain.OffchainTxRequested{
					OffchainTxEvent: domain.OffchainTxEvent{
						Id:   txId,
					...
					UnsignedCheckpointTxs: map[string]string{
						checkpointTxid1: "unsigned1",
						checkpointTxid2: "unsigned2",
					},
				...
				domain.OffchainTxAccepted{
					...
					FinalArkTx: "mock_final_ark_tx",
					SignedCheckpointTxs: map[string]string{
						checkpointTxid1: "signed1",
						checkpointTxid2: "signed2",
					},
					CommitmentTxids: map[string]string{
						checkpointTxid1: rootCommitmentTxid,
						checkpointTxid2: commitmentTxid,
				...
				domain.OffchainTxFinalized{
					...
					FinalCheckpointTxs: map[string]string{
						checkpointTxid1: "final1",
						checkpointTxid2: "final2",
					},
					Timestamp: time.Now().Unix(),
				},
			}

			err = svc.Events().Save(ctx, domain.OffchainTxTopic, txId, events)
			require.NoError(t, err)

			// The projection handler runs asynchronously via a goroutine, so
			// poll until the output vtxo appears in the store.
			require.Eventually(t, func() bool {
				vtxos, err := svc.Vtxos().GetVtxos(ctx, []domain.Outpoint{
					{Txid: outTxid, VOut: 0},
				})
				return err == nil && len(vtxos) > 0
			}, 5*time.Second, 100*time.Millisecond)

			// Verify the output vtxo's Swept flag matches expectations.
			outputVtxos, err := svc.Vtxos().GetVtxos(ctx, []domain.Outpoint{
				{Txid: outTxid, VOut: 0},
			})
			require.NoError(t, err)
			require.Len(t, outputVtxos, 1)
			require.Equal(t, tt.expectSwept, outputVtxos[0].Swept,
				"expected Swept=%v for output vtxo", tt.expectSwept)
		})
	}
}
