...
func TestTokenCache(t *testing.T) {
	...
	t.Run("touch", func(t *testing.T) {
		tests := []struct {
			...
			{
				name: "extends expiry of existing entry",
				setup: func(c *tokenCache) {
					c.add("hash1", []Outpoint{op1, op2}, time.Now())
				},
				assert: func(t *testing.T, c *tokenCache) {
					_, before, ok := c.getOutpoints("hash1")
					require.True(t, ok)

					time.Sleep(10 * time.Millisecond)
					c.touch("hash1")

					_, after, ok := c.getOutpoints("hash1")
					require.True(t, ok)
					require.True(t, after.After(*before))
				},
			},
			{
				name: "extends expiry of every outpoint under the hash",
				setup: func(c *tokenCache) {
					c.add("hash1", []Outpoint{op1, op2}, time.Now())
					time.Sleep(10 * time.Millisecond)
					c.touch("hash1")
				...
				assert: func(t *testing.T, c *tokenCache) {
					c.mu.RLock()
					defer c.mu.RUnlock()
					expiries := make(map[time.Time]struct{})
					for _, expiresAt := range c.outpointsByHash["hash1"] {
						expiries[expiresAt] = struct{}{}
					}
					// touch sets every outpoint to the same new expiry
					require.Len(t, expiries, 1)
				},
			},
			{
				name: "keeps entry alive past its original expiry",
				setup: func(c *tokenCache) {
					c.add("hash1", []Outpoint{op1}, time.Now())
					time.Sleep(30 * time.Millisecond)
					c.touch("hash1") // pushes expiry to ~80ms from start
					time.Sleep(40 * time.Millisecond)
				},
				assert: func(t *testing.T, c *tokenCache) {
					// original 50ms TTL would have expired by now (~70ms),
					// but touch kept it live
					_, _, ok := c.getOutpoints("hash1")
					require.True(t, ok)
				},
			},
			{
				name:  "on unknown hash is a no-op",
				setup: func(c *tokenCache) {},
				assert: func(t *testing.T, c *tokenCache) {
					require.NotPanics(t, func() { c.touch("nonexistent") })
					_, _, ok := c.getOutpoints("nonexistent")
					require.False(t, ok)
				},
			},
		}
	...
	t.Run("isActive", func(t *testing.T) {
		tests := []struct {
			...
			{
				name: "true for a live entry",
				setup: func(c *tokenCache) {
					c.add("hash1", []Outpoint{op1}, time.Now())
				},
				assert: func(t *testing.T, c *tokenCache) {
					require.True(t, c.isActive("hash1"))
				},
			},
			{
				name:  "false for unknown hash",
				setup: func(c *tokenCache) {},
				assert: func(t *testing.T, c *tokenCache) {
					require.False(t, c.isActive("nonexistent"))
				},
			},
			{
				name: "false for an expired entry",
				setup: func(c *tokenCache) {
					c.add("hash1", []Outpoint{op1}, time.Now())
					time.Sleep(100 * time.Millisecond)
				},
				assert: func(t *testing.T, c *tokenCache) {
					require.False(t, c.isActive("hash1"))
				},
			},
			{
				name: "true after touch extends an otherwise expiring entry",
				setup: func(c *tokenCache) {
					c.add("hash1", []Outpoint{op1}, time.Now())
					time.Sleep(30 * time.Millisecond)
					c.touch("hash1")
					time.Sleep(40 * time.Millisecond) // past original 50ms TTL
				},
				assert: func(t *testing.T, c *tokenCache) {
					require.True(t, c.isActive("hash1"))
				},
