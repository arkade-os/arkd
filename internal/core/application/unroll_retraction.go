...
// is retracted. One pass is not enough, since a transaction that has been
// broadcast but has not reached our node yet reads exactly like one that is
// gone, and retracting a live unroll would hand its owner back a vtxo whose
// output exists on chain. Several passes apart make that reading stable rather
// than a race with propagation.
...
// confirmation. That is deliberate, since the mark is protective and blocks the
// vtxo from being spent inside the Ark while its unroll is in flight. Nothing
// else clears it, so an unroll that is evicted or replaced and never mines
// leaves the vtxo wrongly unrolled forever, unspendable by its owner and
// unsweepable by the operator.
//
// Retraction runs on one piece of positive evidence, namely that the node itself
// no longer holds the transaction that would materialise the vtxo, and has not
// mined it.
//
// The wallet's unspent set is deliberately not consulted. It reads as the
// stronger signal, but it is built from the chain backend's own index, which
// keeps an unconfirmed transaction long after the node has dropped it. On a live
// stack with the unroll evicted, the node reported no such output while the
// backend still listed the outpoint as unspent, so consulting it vetoed exactly
// the retraction this exists to make. The node's answer already accounts for
// confirmation, so nothing is lost.
