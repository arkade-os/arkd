...
// but paginating a long VTXO chain can span many requests over a longer period.
// Each successful GetVtxoChain page calls touch so the cache entry stays live;
// validateChainAuth then accepts expired-timestamp tokens as long as the cache
// entry is still active, proving the session was recently used.
