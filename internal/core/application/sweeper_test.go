...
// SignSweepTx came in with the build/sign split. Tests that only prime
// BuildSweepTx are exercising paths where the unsigned tx is enough, so this
// hands the tx back unchanged unless a case primes it.
func (m *mockTxBuilder) SignSweepTx(
	wallet ports.WalletService, unsignedTx string,
) (string, error) {
	if len(m.ExpectedCalls) > 0 {
		for _, c := range m.ExpectedCalls {
			if c.Method == "SignSweepTx" {
...
func newTestSweeper(t *testing.T) (
	...
	*sweeper,
	...
	s := newSweeper(wallet, nil, repoManager, builder, scheduler)
