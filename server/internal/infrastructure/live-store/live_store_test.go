...
var (
	...
	validTx = map[domain.VtxoKey]ports.ValidForfeitTx{
...
func (m *mockedTxBuilder) VerifyForfeitTxs(vtxos []domain.Vtxo, connectors []tree.TxGraphChunk, txs []string) (valid map[domain.VtxoKey]ports.ValidForfeitTx, err error) {
	args := m.Called(vtxos, connectors, txs)
	return args.Get(0).(map[domain.VtxoKey]ports.ValidForfeitTx), args.Error(1)
}
