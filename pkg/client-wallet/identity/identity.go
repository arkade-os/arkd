...
func NewIdentity(store identitystore.IdentityStore) (clientlib.Identity, error) {
	data, err := store.Get()
	if err != nil {
		return nil, err
	}
...
func (s *service) GetType() string {
	return clientlib.SingleKeyIdentity
}
...
func (s *service) Create(
	...
	encryptedPrivateKey, err := encryptAES256(buf, pwd)
	if err != nil {
		return "", err
	}
...
func (s *service) Unlock(
	...
	pwd := []byte(password)
	currentPassHash := hashPassword(pwd)

	if !bytes.Equal(s.data.PasswordHash, currentPassHash) {
		return false, fmt.Errorf("invalid password")
	}

	privateKeyBytes, err := decryptAES256(s.data.EncryptedPrvkey, pwd)
	if err != nil {
		return false, err
	}
...
func (s *service) NewKey(ctx context.Context) (*clientlib.KeyRef, error) {
	...
	return &clientlib.KeyRef{
		Id:     "m",
		PubKey: s.data.PubKey,
	}, nil
}

func (s *service) GetKey(ctx context.Context, _ string) (*clientlib.KeyRef, error) {
	if s.data == nil {
		return nil, ErrNotInitialized
	}
	return &clientlib.KeyRef{
...
func (s *service) ListKeys(ctx context.Context) ([]clientlib.KeyRef, error) {
	key, err := s.GetKey(ctx, "")
	if err != nil {
		return nil, err
	}
	return []clientlib.KeyRef{*key}, nil
}
