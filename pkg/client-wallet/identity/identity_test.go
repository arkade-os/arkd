...
func TestGetKey(t *testing.T) {
	...
	t.Run("invalid", func(t *testing.T) {
		tests := []struct {
			name   string
			setup  func(t *testing.T) clientlib.Identity
			expErr string
		}{
			{
				"not initialized",
				func(t *testing.T) clientlib.Identity { return newTestIdentity(t) },
...
func newTestIdentity(t *testing.T) clientlib.Identity {
	t.Helper()
	store, err := identityinmemorystore.NewStore()
	require.NoError(t, err)
	identitySvc, err := identity.NewIdentity(store)
	require.NoError(t, err)
	return identitySvc
}

func newUnlockedTestIdentity(t *testing.T) (clientlib.Identity, string) {
	t.Helper()
	identitySvc := newTestIdentity(t)
	ctx := t.Context()
