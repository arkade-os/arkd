...
// signerKeyForLeaf returns the wallet's signer key that must sign the given
// tapscript leaf: the current SignerKey or a deprecated one, whichever the leaf's
// multisig closure references.
//
// required is set when the caller explicitly asked for this input to be signed. In
// that case a leaf that references none of the wallet's keys is a hard error,
// usually because a signer key was rotated without being retained as a deprecated
// key, rather than silently signing with the wrong key and failing later at
// finalize. Best-effort callers (required false) fall back to the current key so a
// redundant signature on a leaf the wallet is not part of stays harmless. Leaves
// whose closure cannot be introspected also fall back to the current key.
