...
func (e event) toBatchEvent() (any, error) {
	...
	if ee := e.GetBatchStarted(); ee != nil {
		return clientlib.BatchStartedEvent{
			Id:              ee.GetId(),
	...
	if ee := e.GetBatchFinalization(); ee != nil {
		return clientlib.BatchFinalizationEvent{
			Id: ee.GetId(),
	...
	if ee := e.GetBatchFinalized(); ee != nil {
		return clientlib.BatchFinalizedEvent{
			Id:   ee.GetId(),
	...
	if ee := e.GetTreeSigningStarted(); ee != nil {
		return clientlib.TreeSigningStartedEvent{
			Id:                   ee.GetId(),
	...
	if ee := e.GetTreeNoncesAggregated(); ee != nil {
		...
		return clientlib.TreeNoncesAggregatedEvent{
			Id:     ee.GetId(),
			Nonces: nonces,
		}, nil
	}
	...
	if ee := e.GetTreeNonces(); ee != nil {
		...
		return clientlib.TreeNoncesEvent{
			Id:     ee.GetId(),
			Topic:  ee.GetTopic(),
			Txid:   ee.GetTxid(),
			Nonces: nonces,
	...
	if ee := e.GetTreeSignature(); ee != nil {
		return clientlib.TreeSignatureEvent{
			Id:         ee.GetId(),
			Topic:      ee.GetTopic(),
			BatchIndex: ee.GetBatchIndex(),
			Txid:       ee.GetTxid(),
	...
	if ee := e.GetStreamStarted(); ee != nil {
		return clientlib.StreamStartedEvent{
			Id: ee.GetId(),
		}, nil
	}
...
func (v vtxo) toVtxo() clientlib.Vtxo {
	assetsProto := v.GetAssets()
	assets := make([]clientlib.Asset, 0, len(assetsProto))
...
func (v vtxos) toVtxos() []clientlib.Vtxo {
	list := make([]clientlib.Vtxo, 0, len(v))
	for _, vv := range v {
