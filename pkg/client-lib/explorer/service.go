...
//
//	svc, err := clientlib.NewExplorer("", arklib.Bitcoin, clientlib.WithTracker(true))
//	if err != nil {
//	    log.Fatal(err)
//	}
//	defer svc.Stop()
...
var (
	supportedExplorers = supportedType[string]{
...
// Example:
//
// svc, err := clientlib.NewExplorer("https://mempool.space/api", arklib.Bitcoin, clientlib.WithTracker(true))
func NewExplorer(url string, net arklib.Network, opts ...Option) (clientlib.Explorer, error) {
	baseUrl := url
	if len(baseUrl) <= 0 {
...
func (e *explorerSvc) Start() {
	...
	if err != nil {
		log.WithError(err).WithField("wsURL", wsURL).Debugf(
			"explorer: failed to create connection pool, falling back to polling with interval %s",
			e.pollInterval,
		)
	}
	e.connPoolMu.Lock()
...
func (e *explorerSvc) GetTxs(addr string) ([]clientlib.Tx, error) {
...
func (e *explorerSvc) GetTxOutspends(txid string) ([]clientlib.SpentStatus, error) {
	...
	spentStatuses := make([]clientlib.SpentStatus, 0, len(res))
	for _, s := range res {
		spentStatuses = append(spentStatuses, clientlib.SpentStatus{
			Spent:   s.Spent,
			SpentBy: s.SpentBy,
		})
	}
...
func (e *explorerSvc) GetUtxos(addresses []string) ([]clientlib.ExplorerUtxo, error) {
	if len(addresses) <= 0 {
	...
	allUtxos := make([]clientlib.ExplorerUtxo, 0)
	count := 0
	for addr, script := range addrs {
		...
		// Throttle requests to not overload the clientlib.
		if count%20 == 0 {
		...
		case wsConn := <-connPool.getNewConnections():
			...
			go func(ctx context.Context, wsConn *websocketConnection) {
				if err := wsConn.conn.SetReadDeadline(time.Now().Add(pongInterval)); err != nil {
					if !isCloseError(err) {
						go e.listeners.broadcast(clientlib.OnchainAddressEvent{Error: fmt.Errorf(
					...
					if err := wsConn.conn.ReadJSON(&payload); err != nil {
						...
						if isTimeoutError(err) {
							...
							if err := connPool.resetConnection(wsConn); err != nil {
								go e.listeners.broadcast(clientlib.OnchainAddressEvent{
									Error: fmt.Errorf(
...
func (e *explorerSvc) sendAddressEventFromWs(payload addressNotification) {
	...
	if len(payload.Error) > 0 {
		e.listeners.broadcast(clientlib.OnchainAddressEvent{
	...
	spentUtxos := make([]clientlib.OnchainOutput, 0)
	newUtxos := make([]clientlib.OnchainOutput, 0)
	confirmedUtxos := make([]clientlib.OnchainOutput, 0)
	...
	for addr, data := range payload.MultiAddrTx {
			...
			for _, tx := range data.Mempool {
				for _, in := range tx.Inputs {
					if in.Prevout.Address == addr {
						spentUtxos = append(spentUtxos, clientlib.OnchainOutput{
				...
				for i, out := range tx.Outputs {
					if out.Address == addr {
						...
						newUtxos = append(newUtxos, clientlib.OnchainOutput{
							Outpoint: clientlib.Outpoint{
								Txid: tx.Txid,
								VOut: uint32(i),
							},
		...
		if len(data.Confirmed) > 0 {
			for _, tx := range data.Confirmed {
				for i, out := range tx.Outputs {
					if out.Address == addr {
						confirmedUtxos = append(confirmedUtxos, clientlib.OnchainOutput{
							Outpoint: clientlib.Outpoint{
								Txid: tx.Txid,
								VOut: uint32(i),
							},
...
func (e *explorerSvc) sendAddressEventFromPolling(oldUtxos, newUtxos []utxo) {
	...
	receivedUtxos := make([]clientlib.OnchainOutput, 0)
	confirmedUtxos := make([]clientlib.OnchainOutput, 0)
	for _, newUtxo := range newUtxos {
		oldUtxo, ok := indexedOldUtxos[fmt.Sprintf("%s:%d", newUtxo.Txid, newUtxo.Vout)]
		if !ok {
			utxo := clientlib.OnchainOutput{
				Outpoint: clientlib.Outpoint{
					Txid: newUtxo.Txid,
					VOut: newUtxo.Vout,
				},
...
func (e *explorerSvc) getLegacyFeeRate() (float64, error) {
	...
	if _, err := e.get("fee-estimates", &response); err != nil {
		return 0, fmt.Errorf("failed to get legacy fee rate: %w", err)
	}
...
func (e *explorerSvc) get(path string, target any) (int, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return resp.StatusCode, fmt.Errorf(
			"got unexpected status %d: %s", resp.StatusCode, string(body),
		)
	}

	if err := json.Unmarshal(body, target); err != nil {
		return 0, fmt.Errorf("failed to parse response %q: %w", string(body), err)
	}
