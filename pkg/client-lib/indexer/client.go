...
func NewClient(serverUrl string) (clientlib.Indexer, error) {
	if len(serverUrl) <= 0 {
...
func (a *grpcClient) GetCommitmentTx(
	...
	batches := make(map[uint32]*clientlib.Batch)
	for vout, batch := range resp.GetBatches() {
		batches[vout] = &clientlib.Batch{
...
func (a *grpcClient) GetVtxoTree(
	ctx context.Context, batchOutpoint clientlib.Outpoint, opts ...clientlib.PageOption,
) (*clientlib.VtxoTreeResponse, error) {
	o, err := clientlib.ApplyPageOptions(opts...)
	if err != nil {
		return nil, err
	}
	...
	nodes := make([]clientlib.TxNode, 0, len(resp.GetVtxoTree()))
	for _, node := range resp.GetVtxoTree() {
		nodes = append(nodes, clientlib.TxNode{
			Txid:     node.GetTxid(),
			Children: node.GetChildren(),
		})
	}

	return &clientlib.VtxoTreeResponse{
		Tree: nodes,
...
func (a *grpcClient) GetFullVtxoTree(
	...
	var allTxs clientlib.TxNodes = resp.Tree
	for resp.Page != nil && resp.Page.Next != resp.Page.Total {
		nextPage := &clientlib.PageRequest{Index: resp.Page.Next}
		if o.Page != nil {
			nextPage.Size = o.Page.Size
		}
		resp, err = a.GetVtxoTree(ctx, batchOutpoint, clientlib.WithPage(nextPage))
		if err != nil {
			return nil, err
		}
...
func (a *grpcClient) GetVtxoTreeLeaves(
	ctx context.Context, batchOutpoint clientlib.Outpoint, opts ...clientlib.PageOption,
) (*clientlib.VtxoTreeLeavesResponse, error) {
	o, err := clientlib.ApplyPageOptions(opts...)
	if err != nil {
		return nil, err
	}
	...
	leaves := make([]clientlib.Outpoint, 0, len(resp.GetLeaves()))
	for _, leaf := range resp.GetLeaves() {
		leaves = append(leaves, clientlib.Outpoint{
			Txid: leaf.GetTxid(),
			VOut: leaf.GetVout(),
		})
	}

	return &clientlib.VtxoTreeLeavesResponse{
		Leaves: leaves,
		Page:   parsePage(resp.GetPage()),
	}, nil
}
...
func (a *grpcClient) GetForfeitTxs(
	ctx context.Context, txid string, opts ...clientlib.PageOption,
) (*clientlib.ForfeitTxsResponse, error) {
	o, err := clientlib.ApplyPageOptions(opts...)
	if err != nil {
		return nil, err
	}
	...
	return &clientlib.ForfeitTxsResponse{
		Txids: resp.GetTxids(),
		Page:  parsePage(resp.GetPage()),
	}, nil
}
...
func (a *grpcClient) GetConnectors(
	ctx context.Context, txid string, opts ...clientlib.PageOption,
) (*clientlib.ConnectorsResponse, error) {
	o, err := clientlib.ApplyPageOptions(opts...)
	if err != nil {
		return nil, err
	}
	...
	connectors := make([]clientlib.TxNode, 0, len(resp.GetConnectors()))
	for _, connector := range resp.GetConnectors() {
		connectors = append(connectors, clientlib.TxNode{
			Txid:     connector.GetTxid(),
			Children: connector.GetChildren(),
		})
	}

	return &clientlib.ConnectorsResponse{
		Tree: connectors,
		Page: parsePage(resp.GetPage()),
	}, nil
}
...
func (a *grpcClient) GetVtxos(
	...
	o, err := clientlib.ApplyGetVtxosOptions(opts...)
	if err != nil {
		return nil, err
	}
	...
	return &clientlib.VtxosResponse{
...
func (a *grpcClient) GetVtxoChain(
	ctx context.Context, outpoint clientlib.Outpoint, opts ...clientlib.PageOption,
) (*clientlib.VtxoChainResponse, error) {
	o, err := clientlib.ApplyPageOptions(opts...)
	if err != nil {
		return nil, err
	}
	...
	chain := make([]clientlib.ChainWithExpiry, 0, len(resp.GetChain()))
	for _, c := range resp.GetChain() {
		var txType clientlib.IndexerChainedTxType
		switch c.GetType() {
		case arkv1.IndexerChainedTxType_INDEXER_CHAINED_TX_TYPE_COMMITMENT:
			txType = clientlib.IndexerChainedTxTypeCommitment
		case arkv1.IndexerChainedTxType_INDEXER_CHAINED_TX_TYPE_ARK:
			txType = clientlib.IndexerChainedTxTypeArk
		case arkv1.IndexerChainedTxType_INDEXER_CHAINED_TX_TYPE_TREE:
			txType = clientlib.IndexerChainedTxTypeTree
		case arkv1.IndexerChainedTxType_INDEXER_CHAINED_TX_TYPE_CHECKPOINT:
			txType = clientlib.IndexerChainedTxTypeCheckpoint
		default:
			txType = clientlib.IndexerChainedTxTypeUnspecified
		}

		chain = append(chain, clientlib.ChainWithExpiry{
			Txid:      c.GetTxid(),
			Type:      txType,
	...
	return &clientlib.VtxoChainResponse{
		Chain: chain,
		Page:  parsePage(resp.GetPage()),
	}, nil
}
...
func (a *grpcClient) GetVirtualTxs(
	ctx context.Context, txids []string, opts ...clientlib.PageOption,
) (*clientlib.VirtualTxsResponse, error) {
	o, err := clientlib.ApplyPageOptions(opts...)
	if err != nil {
		return nil, err
	}
	...
	return &clientlib.VirtualTxsResponse{
		Txs:  resp.GetTxs(),
		Page: parsePage(resp.GetPage()),
	}, nil
}
...
func (a *grpcClient) GetBatchSweepTxs(
	ctx context.Context, batchOutpoint clientlib.Outpoint,
) ([]string, error) {
...
func (a *grpcClient) NewSubscription(
...
) (string, <-chan clientlib.ScriptEvent, func(), error) {
	...
	stream, closeFn, err := utils.StartReconnectingStream(ctx, utils.ReconnectingStreamConfig[
		...
		*arkv1.GetSubscriptionResponse,
		...
		HandleResp: func(
			ctx context.Context,
			eventsCh chan<- clientlib.ScriptEvent,
			resp *arkv1.GetSubscriptionResponse,
		) error {
			var checkpointTxs map[string]clientlib.TxData
			...
			case *arkv1.GetSubscriptionResponse_Event:
				...
				if len(event.GetCheckpointTxs()) > 0 {
					checkpointTxs = make(map[string]clientlib.TxData)
					for k, v := range event.GetCheckpointTxs() {
						checkpointTxs[k] = clientlib.TxData{
							Txid: v.GetTxid(),
							Tx:   v.GetTx(),
						}
					}
			...
			case <-ctx.Done():
				return ctx.Err()
			case eventsCh <- clientlib.ScriptEvent{
				Data: &clientlib.ScriptEventData{
					Txid:          event.GetTxid(),
					Tx:            event.GetTx(),
		...
		ErrorEvent: func(err error) clientlib.ScriptEvent {
			return clientlib.ScriptEvent{Err: err}
		},
		ConnectionEvent: func(event utils.ReconnectingStreamStateEvent) clientlib.ScriptEvent {
			return clientlib.ScriptEvent{
...
func (a *grpcClient) paginatedGetVirtualTxs(
	...
	return &clientlib.VirtualTxsResponse{Txs: txs}, nil
}
...
func parsePage(page *arkv1.IndexerPageResponse) *clientlib.PageResponse {
	if page == nil {
		return nil
	}
	return &clientlib.PageResponse{
...
func newIndexerVtxos(vtxos []*arkv1.IndexerVtxo) []clientlib.Vtxo {
	res := make([]clientlib.Vtxo, 0, len(vtxos))
...
func newIndexerVtxo(vtxo *arkv1.IndexerVtxo) clientlib.Vtxo {
	var assetLists []clientlib.Asset
	for _, a := range vtxo.GetAssets() {
		if a != nil {
			assetLists = append(assetLists, clientlib.Asset{
