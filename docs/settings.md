...
| `ARKD_RATE_LIMIT_MAX_VELOCITY`           | `rate_limit_max_velocity`        | max VTXO chain growth in depths per second before rejecting (must be `> 0` when enabled) | `0.28`      |
| `ARKD_RATE_LIMIT_MAX_COOLDOWN_SECS`      | `rate_limit_max_cooldown_secs`   | seconds; cap on the cooldown suggested to rejected clients          | `3600`      |
