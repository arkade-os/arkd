...
| `ARKD_RATE_LIMIT_MAX_VELOCITY`     | Max VTXO chain growth, in depths per second, before an input is rate limited (must be `> 0` when enabled) | `0.28`                         |
| `ARKD_RATE_LIMIT_MAX_COOLDOWN_SECS`| Cap (in seconds) on the cooldown suggested to rate-limited clients              | `3600`                         |
